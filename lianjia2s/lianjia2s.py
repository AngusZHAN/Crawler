...
def get_url(url, page):
    ...
    browser.get(url + '/pg%s' % page)
    html = browser.page.source
    table = BeautifulSoup(html, "lxml").find(
        'div', {'class': 'mod_cont'}).find_all('li', {'class': 'pictext'})
    result = []
    ...
    for li in table:
        ...
        item['title'] = li.find(
            'div', {'class': 'item_main text_cut'}).get_text
        result.append(item)
...
def get_info(item):
    ...
    item['house_price'] = info_box.find(
        'div', {'class': 'house_price'}).get_text()
    item['tags'] = info_box.find('div', {'class': 'tag_box'}).get_text()
    tits = soup.find('ul', {'class': 'lists'}).find_all(
        'div', {'class': 'tit'})
    values = soup.find('ul', {'class': 'list'}).find_ll(
        'div', {'class': 'value box_col'})

    for index in range(len(tits)):
        item[tits[index].get_text().replace('\r', '').replce('\n', '').replace(
            ' ', '').replce(': ', '')] = values[index].get_text()
    try:
        item['买点'] = soup.find('p', {'class': 'text'}).get_text
    except:
        pass
    try:
        communityurl = 'http://m.lianjia.com/' + \
            soup.find('ul', {'class': 'lists'}).find(
                'a', {'class': 'flexbox'}).get('href')
    except:
        return item
    try:
        ...
        values = house_detail.find_all('spna', {'class': 'value box_col'})
    ...
    while True:
        ...
        for item in result:
            try:
                ...
                failed = open('failed.text', 'a', enconding='utf-8')
...
def write_to_excel():
    ...
    keys = ['title', 'url', 'house_price', 'tags', '面积', '首付', '月供', '朝向', '楼型', '装修', '房源编号', '楼层',
            '户型', '年代', '地铁', '卖点', '小区', '建造年代', '均价', '物业费用', '房屋类型', '绿化率', '楼栋数', '建造类型', '容积率', '房屋总数']
    sheet.append(keys)
